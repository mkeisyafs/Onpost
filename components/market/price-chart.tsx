...
export function PriceChart({ snapshot }: PriceChartProps) {
  ...
  // Calculate scale
  const allPrices = [sell.p10, sell.median, sell.p90, buy.p10, buy.median, buy.p90].filter(
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">{sell.count} listings</span>
        </div>
        <div className="relative h-8 rounded-lg bg-secondary">
          ...
          <div
            ...
            title={`Median: ${formatPrice(sell.median, "IDR")}`}
          />
        </div>
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>P10: {formatPrice(sell.p10, "IDR")}</span>
          <span className="font-medium text-wts">Median: {formatPrice(sell.median, "IDR")}</span>
          <span>P90: {formatPrice(sell.p90, "IDR")}</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-muted-foreground">{buy.count} listings</span>
        </div>
        <div className="relative h-8 rounded-lg bg-secondary">
          {/* P10-P90 Range */}
          <div
          ...
          <div
            ...
            title={`Median: ${formatPrice(buy.median, "IDR")}`}
          />
        </div>
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>P10: {formatPrice(buy.p10, "IDR")}</span>
          <span className="font-medium text-wtb">Median: {formatPrice(buy.median, "IDR")}</span>
          <span>P90: {formatPrice(buy.p90, "IDR")}</span>
        </div>
      </div>

      {/* Price Scale */}
      <div className="flex justify-between border-t border-border pt-4 text-xs text-muted-foreground">
        <span>{formatPrice(minPrice, "IDR")}</span>
        <span>{formatPrice((minPrice + maxPrice) / 2, "IDR")}</span>
        <span>{formatPrice(maxPrice, "IDR")}</span>
      </div>
    </div>
  )
}
