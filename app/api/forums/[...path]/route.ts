...
async function proxyRequest(request: NextRequest, params: { path: string[] }) {
  try {
    ...
    if (FORUMS_API_KEY) {
      ...
      console.warn(
        "[Proxy] FORUMS_API_KEY not set - some endpoints may require authentication"
      );
    }
    ...
    // Log errors from upstream API for debugging
    if (!response.ok) {
      console.error(
        `[Proxy] Upstream API error: ${response.status} ${url.pathname}${url.search}`,
