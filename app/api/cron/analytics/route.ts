...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const threadsResponse = await forumsRequest<{ threads: ForumsThread[] }>("/threads?filter=newest&limit=20")

    const marketThreads = threadsResponse.threads.filter((t) => t.extendedData?.market?.marketEnabled)
    ...
    for (const thread of marketThreads.slice(0, 10)) {
      try {
        ...
        const shouldRescan = Date.now() - market.lastWindowCutoffAt > 60 * 60 * 1000 // Rescan hourly max
        let validCount = market.validCount

        if (shouldRescan || newPosts.length > 0) {
          validCount = await countValidTradesInWindow(thread.id, market.windowDays)
        }
        ...
        const updatedMarket: ThreadMarketData = {
          ...market,
          validCount,
          lastWindowCutoffAt: shouldRescan ? Date.now() : market.lastWindowCutoffAt,
          lastProcessed: {
            mode: "NEWEST",
            cursor: newPosts[0]?.id ? null : market.lastProcessed.cursor,
            lastPostIdProcessed: newPosts[0]?.id || market.lastProcessed.lastPostIdProcessed,
            at: Date.now(),
          },
        }

        if (validCount >= market.thresholdValid) {
          const marketType = market.marketTypeFinal || market.marketTypeCandidate
          const snapshot = await computeSnapshot(thread.id, marketType, market.windowDays)
          ...
          if (shouldRefreshNarrative(previousSnapshot, snapshot)) {
            try {
              const narrativeResponse = await fetch(`${process.env.VERCEL_URL || ""}/api/ai/narrative`, {
                method: "POST",
        ...
        results.errors.push(`Thread ${thread.id}: ${error instanceof Error ? error.message : "Unknown error"}`)
      }
    }
...
async function forumsRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
...
async function fetchNewPosts(
  ...
  while (true) {
    const response = await forumsRequest<{ posts: ForumsPost[]; nextPostCursor: string | null }>(
      `/thread/${threadId}/posts?filter=newest${cursor ? `&cursor=${cursor}` : ""}`,
    )
...
async function processPostForTrade(post: ForumsPost, threadId: string): Promise<void> {
  const existingTrade = post.extendedData?.trade

  // Skip if already has trade data with good confidence and price
  if (existingTrade && existingTrade.parseConfidence >= 0.7 && existingTrade.normalizedPrice !== null) {
    return
  }
  ...
  if (!tradeData || tradeData.parseConfidence < 0.7 || tradeData.normalizedPrice === null) {
    try {
      const aiResponse = await fetch(`${process.env.VERCEL_URL || ""}/api/ai/parse`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function countValidTradesInWindow(threadId: string, windowDays: number): Promise<number> {
  ...
  while (true) {
    ...
    for (const post of response.posts) {
      ...
      if (trade?.isTrade && trade.status === "ACTIVE" && trade.normalizedPrice !== null) {
        validCount++
      }
    }
...
function computeItemMarketSnapshot(trades: PostTradeData[]): MarketSnapshot {
  const wtsPrices = trades.filter((t) => t.intent === "WTS").map((t) => t.normalizedPrice!)
  const wtbPrices = trades.filter((t) => t.intent === "WTB").map((t) => t.normalizedPrice!)
...
function computeAccountMarketSnapshot(trades: PostTradeData[]): AccountMarketSnapshot {
  ...
  return {
    bands: {
      ...
      premium: { median: median(premium), count: premium.length, range: [q3, Number.POSITIVE_INFINITY] },
    },
...
function shouldRefreshNarrative(
  ...
  if ("sell" in current && "sell" in prev) {
    ...
    const medianChange = Math.abs(current.sell.median - prev.sell.median) / prev.sell.median
    return medianChange > 0.1
  }

  if ("bands" in current && "bands" in prev) {
    // Account market: refresh if total count changed significantly
    const countChange = Math.abs(current.totalValidCount - prev.totalValidCount) / prev.totalValidCount
...
function hasHighLikelihoodTradePattern(body: string): boolean {
  const HIGH_LIKELIHOOD_PATTERN = /\b(WTS|WTB|WTT|SELL|BUY|JUAL|BELI|DIJUAL|CARI|S>|B>|T>)\b/i
  const PRICE_INDICATOR_PATTERN = /(\d+(?:[.,]\d+)?)\s*(k|rb|ribu|jt|juta|m|million|\$|usd|rp)/i
  return HIGH_LIKELIHOOD_PATTERN.test(body) && PRICE_INDICATOR_PATTERN.test(body)
}
...
function parsePrice(body: string): {
  ...
  const PRICE_PATTERNS = [
    { regex: /(\d+(?:[.,]\d+)?)\s*(jt|juta)/i, multiplier: 1_000_000, currency: "IDR" },
    { regex: /(\d+(?:[.,]\d+)?)\s*(rb|ribu)/i, multiplier: 1_000, currency: "IDR" },
    { regex: /(\d+(?:[.,]\d+)?)\s*k\b/i, multiplier: 1_000, currency: "IDR" },
    { regex: /\$\s*(\d+(?:[.,]\d+)?)/i, multiplier: 1, currency: "USD" },
    { regex: /(\d+(?:[.,]\d+)?)\s*(usd|dollar)/i, multiplier: 1, currency: "USD" },
  ]
  ...
  for (const { regex, multiplier, currency } of PRICE_PATTERNS) {
    ...
    if (match) {
      ...
      if (!isNaN(value) && value > 0) {
        return { price: value, displayPrice: match[0], currency, confidence: 0.85 }
      }
    }
  }
