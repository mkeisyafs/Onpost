...
export default function NewThreadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (bodyTrimmed.length > BODY_MAX_LENGTH) {
      setError(
        `Content must be ${BODY_MAX_LENGTH.toLocaleString()} characters or less`
      );
      return;
    }
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className={`resize-none ${
                  bodyTrimmed.length > 0 && !isBodyValid
                    ? "border-destructive"
                    : ""
                }`}
                required
              ...
              {bodyTrimmed.length > 0 &&
                bodyTrimmed.length < BODY_MIN_LENGTH && (
                  <p className="text-xs text-destructive">
                    Content must be at least {BODY_MIN_LENGTH} characters
                  </p>
                )}
            </div>
            ...
            {enableMarket && (
              <div className="rounded-lg border border-green-500/30 bg-green-500/5 p-3">
                <p className="text-sm text-green-600 dark:text-green-400">
                  ✓ Market analytics will track{" "}
                  <strong>
                    {
                      threadCategories.find((c) => c.value === threadCategory)
                        ?.label
                    }
                  </strong>{" "}
                  prices
