...
const PRICE_PATTERNS: Array<{
  ...
  {
    regex: /(?:rp\.?\s*)?(\d{1,3}(?:[.,]\d{3})+)(?!\s*(?:jt|juta|rb|ribu|k|usd|dollar|\$))/i,
    multiplier: 1,
    currency: "IDR",
  },
  // USD patterns
  ...
  { regex: /(?:price|harga|@)\s*:?\s*(\d+(?:[.,]\d+)?)/i, multiplier: 1, currency: "IDR" },
]
  ...
  for (const { regex, multiplier, currency } of PRICE_PATTERNS) {
    ...
    if (match) {
      ...
      if (numStr.includes(".") && numStr.includes(",")) {
        ...
        if (decimalMatch && decimalMatch[1].length === 2 && currency === "USD") {
...
export function createTradeData(body: string, existingData?: Partial<PostTradeData>): PostTradeData | null {
