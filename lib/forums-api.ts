...
export const auth = {
  async login(data: LoginRequest): Promise<{ user: ForumsUser; token: string }> {
  ...
  async resetPassword(data: { password: string; oldPassword?: string; email?: string }): Promise<{ message: string }> {
...
export const posts = {
  async list(
    ...
    const query = searchParams.toString()
    return request<PostsResponse>(`/thread/${threadId}/posts${query ? `?${query}` : ""}`)
  },
...
export const users = {
  ...
  async getPosts(
    ...
    const query = searchParams.toString()
    return request<PostsResponse>(`/user/${userId}/posts${query ? `?${query}` : ""}`)
  },
  ...
  async getThreads(
    ...
    const query = searchParams.toString()
    return request<ThreadsResponse>(`/user/${userId}/threads${query ? `?${query}` : ""}`)
  },
}
...
export const messages = {
  async list(params?: {
    ...
    const query = searchParams.toString()
    return request<MessagesResponse>(`/private-messages${query ? `?${query}` : ""}`)
  },
...
export const search = {
  async threads(query: string): Promise<ThreadsResponse> {
    return request<ThreadsResponse>(`/search/threads?q=${encodeURIComponent(query)}`)
  },

  async posts(query: string): Promise<PostsResponse> {
    return request<PostsResponse>(`/search/posts?q=${encodeURIComponent(query)}`)
  },

  async users(query: string): Promise<ForumsUser[]> {
    return request<ForumsUser[]>(`/search/users?q=${encodeURIComponent(query)}`)
  },
}
